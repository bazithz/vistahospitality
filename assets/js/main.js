    ...
    t(window).on("resize", function () {
      (t("[data-sec-pos]").each(function () {
        ...
        (t(e).css("padding-top", t(e).attr("data-original-padding-top")),
          t(e).css(
            "padding-bottom",
            t(e).attr("data-original-padding-bottom"),
          ));
      }),
        d());
    }),
...
(function () {
  const carousel = document.getElementById('vistaCarousel');
  const AUTOPLAY_MS = 10000;

  function createParticles() {
    const particleContainers = carousel.querySelectorAll('.particles');
  ...
  function resetSlide(slide) {
    ...
    bannerContent.querySelectorAll('h2, p, a').forEach(el => {
      el.classList.remove('animate__fadeInDown', 'animate__fadeInUp', 'animate__bounceIn');
    });
  ...
  function animateSlide(slide) {
    ...
    const h2 = bannerContent.querySelector('h2');
    const p = bannerContent.querySelector('p');
    const btn = bannerContent.querySelector('a');

    if (h2) { h2.classList.add('animate__fadeInDown'); h2.style.animationDelay = '0.3s'; }
    if (p) { p.classList.add('animate__fadeInUp'); p.style.animationDelay = '0.6s'; }
    if (btn) { btn.classList.add('animate__bounceIn'); btn.style.animationDelay = '0.9s'; }
  }
  ...
  function restartProgress(slide) {
    ...
    progress.style.animation = `slideProgress ${AUTOPLAY_MS}ms linear forwards`;
  }
  ...
  function bindParallax() {
    document.addEventListener('mousemove', function(e) {
      ...
      const shapeWrappers = carousel.querySelectorAll('.vista-shape-wrp');
  ...
  // Animate every slide change (autoplay or manual navigation)
  carousel.addEventListener('slid.bs.carousel', handleSlideChange);

  // Button hover effect
  carousel.addEventListener('mouseenter', function(e) {
    if (e.target.classList.contains('btn')) e.target.classList.add('animate__pulse');
  });
  carousel.addEventListener('mouseleave', function(e) {
    if (e.target.classList.contains('btn')) e.target.classList.remove('animate__pulse');
  });
})();
